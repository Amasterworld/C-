...
// constructor
PathFinding_DPS::PathFinding_DPS()
{
...
void PathFinding_DPS::DrawMap(const auto& p)
{
	// Draw Map
	Clear(olc::BLACK);
	...
	DrawString(540, 200 + nDistBetLines * 0, "Depth First Search", olc::WHITE, 1);
	DrawString(540, 200 + nDistBetLines * 1, "Cannot GUARRANTE to get the shortest path", olc::WHITE, 1);
...
void PathFinding_DPS::SetUp(bool Start, std::string sIni, std::string sLoadingDefaultMaze_1, std::string sLoadingDefaultMaze_2, auto p)
{
	if (!bStartButton)
	{
...
void PathFinding_DPS::DrawBorder(const auto p)
{
	for (int x = 0; x < nMapWidth; x++)
	{
		for (int y = 0; y < nMapHeight; y++)
...
void PathFinding_DPS::DFS(bool& bReach, bool& bStartButton, auto p)
{
	if (!bReach && bStartButton)
	{
		...
		if ((nCurrY - 1) >= 0 && nFlowFieldZ[p(nCurrX, nCurrY - 1)] != -1 && bVisited_algo[p(nCurrX, nCurrY - 1)] == false)
		{
			...
			//std::cout << nCurrY - 1 << "    " << nCurrX << "   " << bVisited_algo[nCurrX][nCurrY - 1] << std::endl;
		...
		{
			//std::cout << "bVisited_algo[p(nCurrX, nCurrY - 1) " << bVisited_algo[p(nCurrX, nCurrY - 1)] << std::endl;

			//std::cout << " draw 4" << std::endl;


			neighbours.emplace_back(3);
			//lReachPath.emplace_back(nCurrX + 1, nCurrY);
			//bVisited_algo[p(nCurrX + 1, nCurrY)] = true;
		...
		if (!neighbours.empty())
		{
			// choose random direction from 4 dir
			int next_possible_cell = neighbours[rand() % neighbours.size()];
			switch (next_possible_cell)
			{
			case 0: // up
		...
		//	std::cout << "Path empty" << std::endl;
			//DrawString(600, 200 + nDistBetLines * 8, "PATH IS NOT FOUND", olc::GREEN, 2);
			//DrawString(520, 200 + nDistBetLines * 10, "No path is created", olc::RED, 1);
	...
	if (path.empty() == false)
	{
		for (int x = 0; x < nMapWidth; x++)
			for (int y = 0; y < nMapHeight; y++)
			{
				...
				FillCircle(path.top().first * nCellSize + ((nCellSize - nBorderWidth) / 2), path.top().second * nCellSize + ((nCellSize - nBorderWidth) / 2), 10, olc::YELLOW);
			}
	}
}

void PathFinding_DPS::DrawPathAndlength(bool bReach, std::string sIni)
{
	if (bReach)
	{
		sIni = " ";
		...
		if (bReach && pqTrajec.empty())
		{
			...
			DrawString(520, 200 + nDistBetLines * 10, "The lenght of a path is", olc::YELLOW, 2);
			DrawString(650, 200 + nDistBetLines * 12, std::to_string(nMaxDistStart2EndPoint), olc::YELLOW, 2);
			Reset();
...
void PathFinding_DPS::InputMouse(const auto p)
{
